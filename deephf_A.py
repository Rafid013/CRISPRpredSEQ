...
rf = RandomForestRegressor(n_estimators=500, n_jobs=-1, random_state=1, verbose=2)

steps = [('SFM', SelectFromModel(estimator=rf, max_features=2899, threshold=-np.inf)),
         ('scaler', StandardScaler()),
         ('SVM', SVR(C=1, gamma='auto', kernel='rbf', cache_size=20000, verbose=True,
                                max_mem_size=6000))]
...
f = open('DEEPHF Models/A_sfm_' + str(cas9_type) + '.pkl', 'wb')
pkl.dump(sfm, f)

f = open('DEEPHF Models/A_rf_' + str(cas9_type) + '.pkl', 'wb')
pkl.dump(trained_rf, f)

f = open('DEEPHF Models/A_scaler_' + str(cas9_type) + '.pkl', 'wb')
pkl.dump(scaler, f)

svm.save_to_file('DEEPHF Models/A_svm_' + str(cas9_type) + '.txt')
