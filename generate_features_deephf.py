...
data = pd.read_excel('deephf_data.xlsx', skiprows=1)

sgRNAs = data['sgRNA']
labels1 = data['Wt_Efficiency']
labels2 = data['eSpCas 9_Efficiency']
labels3 = data['SpCas9-HF1_Efficiency']
...
df_main.to_hdf('DEEPHF/data_x.h5', key='deephf')
labels1.to_hdf('DEEPHF/data_y1.h5', key='deephf')
labels2.to_hdf('DEEPHF/data_y2.h5', key='deephf')
labels3.to_hdf('DEEPHF/data_y3.h5', key='deephf')

df_main = pd.concat([df_pos_ind, df_pos_spe], axis=1, sort=False).astype(np.int8)
df_main.to_hdf('DEEPHF/data_without_gapped_x.h5', key='deephf')
