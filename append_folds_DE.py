        ...
        for leave_out_cell in cells:
            for cell in cells:
                if cell != leave_out_cell:
                    key = cell
                    train_x = train_x.append(pd.DataFrame(pd.read_hdf('Folds/train_x_' + gapped + cell.lower() +
                                                                      '_' + str(i) + '.h5', key=key)),
                                             ignore_index=True).astype(np.int8)
                    train_y = train_y.append(pd.DataFrame(pd.read_hdf('Folds/train_y_' + gapped + cell.lower() +
                                                                      '_' + str(i) + '.h5', key=key)),
                                             ignore_index=True).astype(np.int8)
